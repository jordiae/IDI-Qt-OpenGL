...
{
  // Afegit per l'exercici 2.3.4:
   
  float rav = float(w)/float(h);

  // Farem servir -4 i 4 perque son els valors inicials
  // que havia donat a l, r, t, b
  // si rav > 1, cal incrementar la raw => modificar window.
  // Augmentem amplada
  if (rav > 1) {
    left = -4*rav;
    right = 4*rav;
  }
  // Raonament similar (transparencies). Augmentem h
  else if (rav < 1) {
    top = 4/rav;
    bottom = -4/rav;
  } 
...
void MyGLWidget::projectTransform() {
  ...
  // Canviem aixo ultim per l'exercici 2.3.3
...
void MyGLWidget::ini_camera() {
  ...
  ra = 1.0f;
  znear = 0.4f;
  zfar = 10; // canviat a 10 des de 3.0f pel 2.3.4
  projectTransform();
  viewTransform();
}
