...
void MyGLWidget::initializeGL ()
{
  ...
  calcul_punts_capsa_lego();
  capsa_contenidora_lego(punt_min_lego,punt_max_lego);
...
void MyGLWidget::createBuffers () 
{
  ...
  // Pel 2.4.2, copiem el codi de Patricio, pero pel Legoman
  ...
  // pero no calien 2 VAOS ni VBOS per fer els dos Patricios, ni legomans!
...
void MyGLWidget::tractaCanviFOV(int angle) {
  // Map aproximat de 0 a 99 a 0.1 a M_PI/2 - 0.1

  // Realment hauria de ser (0,M_PI/2), pero deixo el 0.1 de marge
  // per evitar els valors extrems

  // Altra manera (millor?) hagues sigut: (float) (i*M_PI*2)/360.0f
  // Trobem el map aixi: M_PI/2 - 0.1 = 0.1 + k*99
  float n_angle = 0.1 + (float)angle *((M_PI/2-0.1 -0.1)/99);
  makeCurrent();
  alpha = n_angle;
  FOV = 2*alpha;
  projectTransformPostZoom();
  update();
  std::cout << "alpha: " << n_angle << std::endl;
}

void MyGLWidget::patricioClicat(){
  model_seleccionat = pat;
  std::cout << "pat seleccionat " << std::endl;
  update();
}
void MyGLWidget::legomanClicat(){
  model_seleccionat = lego;
  std::cout << "lego seleccionat " << std::endl;
  update();
}
...
void MyGLWidget::modelTransformLegoman() {
  glm::mat4 transform (1.0f);
  transform = glm::translate(transform,glm::vec3(1,0,1));
  transform = glm::scale(transform, glm::vec3(1/altura_lego));
  transform = glm::translate(transform,glm::vec3(0,altura_lego/2,0));
  transform = glm::translate(transform,glm::vec3(-centre_capsa_contenidora_lego.x,-centre_capsa_contenidora_lego.y,-centre_capsa_contenidora_lego.z));
  glUniformMatrix4fv(transLoc, 1, GL_FALSE, &transform[0][0]);
}
...
void MyGLWidget::calcul_punts_capsa_lego() {
  // Recorrem tots els vertex buscant el maxim i el minim
  // Anirem de 3 en 3 perque per cada iteracio mirarem
  // els 3 vertexs, x, y, z
  punt_min_lego.x = punt_max_lego.x = legoman.vertices()[0];
  punt_min_lego.y = punt_max_lego.y = legoman.vertices()[1];
  punt_min_lego.z = punt_max_lego.z = legoman.vertices()[2];
  unsigned int i = 3; // evitem warning. .size() es unsigned
  while (i < legoman.vertices().size()) {
    float aux_x = legoman.vertices()[i];
    float aux_y = legoman.vertices()[i+1];
    float aux_z = legoman.vertices()[i+2];
    if (aux_x > punt_max_lego.x)
      punt_max_lego.x = aux_x;
    else if (aux_x < punt_min_lego.x)
      punt_min_lego.x = aux_x;
    if (aux_y > punt_max_lego.y)
      punt_max_lego.y = aux_y;
    else if (aux_y < punt_min_lego.y)
      punt_min_lego.y = aux_y;
    if (aux_z > punt_max_lego.z)
      punt_max_lego.z = aux_z;
    else if (aux_z < punt_min_lego.z)
      punt_min_lego.z = aux_z;
    i += 3;
  }
  altura_lego = punt_max_lego.y - punt_min_lego.y;
  std::cout << "Punt min: " << punt_min_lego.x << " " << punt_min_lego.y << " " << punt_min_lego.z;
  std::cout << "Punt max: " << punt_max_lego.x << " " << punt_max_lego.y << " " << punt_max_lego.z;
}

void MyGLWidget::capsa_contenidora_lego(glm::vec3 p_min, glm::vec3 p_max) {
  /*centre_capsa_contenidora = glm::vec3((punt_max.x-punt_min.x)/2 + punt_min.x,(punt_max.y-punt_min.y)/2 + punt_min.y,(punt_max.z-punt_min.z)/2 + punt_min.z);
  radi_esfera = (punt_max.y - centre_capsa_contenidora.y);*/
  centre_capsa_contenidora_lego = glm::vec3((p_max.x+p_min.x)/2,(p_max.y+p_min.y)/2,(p_max.z+p_min.z)/2);
  radi_esfera_lego = distancia_punts(p_max,p_min)/2;

  std::cout << centre_capsa_contenidora_lego.x << " " << centre_capsa_contenidora_lego.y << " " <<centre_capsa_contenidora_lego.z << " " << radi_esfera_lego << std::endl;
}


// emit nom_signal(parametres)