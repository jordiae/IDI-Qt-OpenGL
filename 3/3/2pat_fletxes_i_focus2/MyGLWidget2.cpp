...
//  posFocusSCO = glm::vec3(View*glm::vec4(posFocus, 1.0));
  
  if (llumCamera) {
    posFocus = glm::vec3(0, 0, 0);
    posFocusSCO = posFocus;
  }
  else {
    posFocus = glm::vec3(0, 4, 0);
    posFocusSCO = glm::vec3(View*glm::vec4(posFocus, 1.0));
  }
  ...
  glUniform3f(llumAmbientLoc, 0.2, 0.2, 0.2);
  glUniform3f(posFocusLoc, 1, 0, 1); // si volguessim que fos amb SCO en comptes de SCA, multipliquem per la view matrix
  */
}
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG, glm::vec3(move_x,0,move_z));


  TG = glm::scale(TG, glm::vec3(0.3, 0.3, 0.3));
  TG = glm::scale(TG, glm::vec3(escala, escala, escala));
  // explicacio: hem de multiplicar per altura/0.3 perque volem que
  // tingui altura 0.3. escala = 2/altura (veure calcul). 
  // El 2 surt de "TG = glm::translate(TG, glm::vec3(0,-2/escala,0));"

  //TG = glm::scale(TG, glm::vec3(escala, escala, escala));

  TG = glm::translate(TG, glm::vec3(0,-2/escala,0));
...
// Luz de aplicación, multiplicamos por view
//    posFocusSCO = glm::vec3(View*glm::vec4(posFocus, 1.0));
    if (not llumCamera) {
      posFocusSCO = glm::vec3(View*glm::vec4(posFocus, 1.0));
  ...
  // Luz de cámara, no multiplicamos por view
    else {
      posFocusSCO = posFocus;
      glUniform3fv(posFocusLoc,1, &posFocusSCO[0]);
    }
